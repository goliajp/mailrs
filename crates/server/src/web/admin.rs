...
#[derive(Deserialize)]
pub(super) struct CreateOAuthClientRequest {
    pub name: String,
    #[serde(default)]
...
pub(super) async fn create_oauth_client(
    AuthUser {
        ref permissions,
        ref address,
        ..
    ...
    if !permissions.has("admin.oauth_clients") {
        return (StatusCode::FORBIDDEN, Json(serde_json::json!({"success": false, "message": "insufficient permissions"}))).into_response();
    }
    if req.name.is_empty() || req.name.len() > super::MAX_ADMIN_FIELD_LEN {
        return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"success": false, "message": "invalid name length"}))).into_response();
    }
    ...
    let Some(ref pool) = state.pg_pool else {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"success": false, "message": "database not configured"}))).into_response();
    };
    ...
    {
        Ok((client_id, secret)) => {
            tracing::info!(client = client_id.as_str(), name = req.name.as_str(), "oauth client created");
        ...
        Err(e) => {
            ...
            (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"success": false, "message": "operation failed"}))).into_response()
        }
    }
}

pub(super) async fn list_oauth_clients(
    AuthUser { ref permissions, .. }: AuthUser,
    State(state): State<Arc<WebState>>,
) -> impl IntoResponse {
    if !permissions.has("admin.oauth_clients") {
    ...
    let clients = crate::oidc_store::list_clients(pool).await.unwrap_or_default();
...
pub(super) async fn delete_oauth_client(
