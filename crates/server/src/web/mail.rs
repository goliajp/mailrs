...
#[derive(Deserialize)]
pub(super) struct SpamFeedbackRequest {
...
pub(super) async fn submit_spam_feedback(
    AuthUser { address: user, .. }: AuthUser,
    State(state): State<Arc<WebState>>,
    Json(req): Json<SpamFeedbackRequest>,
) -> impl IntoResponse {
    if req.label != "spam" && req.label != "ham" {
        return Json(ApiResult { success: false, message: Some("label must be spam or ham".into()) }).into_response();
    }
    let Some(ref pool) = state.pg_pool else {
    ...
    .await
    {
        Ok(_) => Json(ApiResult { success: true, message: None }).into_response(),
        Err(e) => Json(ApiResult { success: false, message: Some(e.to_string()) }).into_response(),
    }
}

pub(super) async fn get_spam_feedback_stats(
    ...
    let stats: Vec<(String, i64)> = sqlx::query_as(
        "SELECT label, COUNT(*) FROM spam_feedback GROUP BY label"
    )
    .fetch_all(pool)
    .await
    .unwrap_or_default();

    let total_spam = stats.iter().find(|(l, _)| l == "spam").map(|(_, c)| *c).unwrap_or(0);
    let total_ham = stats.iter().find(|(l, _)| l == "ham").map(|(_, c)| *c).unwrap_or(0);
