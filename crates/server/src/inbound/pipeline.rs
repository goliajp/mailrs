...
pub async fn run_inbound_pipeline(
            ...
            match policy {
                mail_auth::dmarc::Policy::Reject => {
                    ...
                    if let Some(store) = dmarc_report_store {
                        let _ = store.record_result(&DmarcResultRecord {
                    ...
                    let auth_header = build_auth_header(
                        hostname, &spf_str, &dkim_str, &arc_str, "fail", Some("policy=reject"),
                    );
                    tracing::info!(
                        event = "dmarc_reject",
    ...
    let ai_score = if rule_total > 1.0 && rule_total < spam_score_threshold {
        if let Some(ai_cfg) = ai_config {
            ...
            match crate::ai_spam::classify(ai_cfg, valkey, sender, &subject, &body_preview).await {
