...
fn make_msg(
    overrides: impl FnOnce(&mut mailrs_mailbox::MessageMeta),
) -> mailrs_mailbox::MessageMeta {
...
fn matches_to_case_insensitive() {
    ...
    assert!(message_matches_criteria(
        &msg,
        &[SearchKey::To("bob".into())]
    ));
    assert!(!message_matches_criteria(
        &msg,
        &[SearchKey::To("alice".into())]
    ));
}
...
fn matches_text_searches_multiple_fields() {
    ...
    assert!(message_matches_criteria(
        &msg,
        &[SearchKey::Text("alice".into())]
    ));
    assert!(message_matches_criteria(
        &msg,
        &[SearchKey::Text("bob".into())]
    ));
    assert!(message_matches_criteria(
        &msg,
        &[SearchKey::Text("important".into())]
    ));
    assert!(!message_matches_criteria(
        &msg,
        &[SearchKey::Text("charlie".into())]
    ));
}
...
fn matches_since_before_on() {
    ...
    assert!(message_matches_criteria(
        &msg,
        &[SearchKey::Since(1699999999)]
    ));
    assert!(message_matches_criteria(
        &msg,
        &[SearchKey::Since(1700000000)]
    ));
    assert!(!message_matches_criteria(
        &msg,
        &[SearchKey::Since(1700000001)]
    ));

    assert!(message_matches_criteria(
        &msg,
        &[SearchKey::Before(1700000001)]
    ));
    assert!(!message_matches_criteria(
        &msg,
        &[SearchKey::Before(1700000000)]
    ));

    assert!(message_matches_criteria(&msg, &[SearchKey::On(1700000000)]));
    assert!(!message_matches_criteria(
        &msg,
        &[SearchKey::On(1700000000 + 86400)]
    ));
}
...
fn matches_uid_search() {
    ...
    assert!(message_matches_criteria(
        &msg,
        &[SearchKey::Uid("42".into())]
    ));
    assert!(message_matches_criteria(
        &msg,
        &[SearchKey::Uid("40:45".into())]
    ));
    assert!(!message_matches_criteria(
        &msg,
        &[SearchKey::Uid("1:10".into())]
    ));
}
