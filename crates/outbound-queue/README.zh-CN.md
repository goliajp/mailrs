...
- **Postgres 参考实现** —— [`PgQueueStore`] + [`KevyNotifier`] (默认 `pg` feature) 直接对接 mailrs 用的 schema, 一个构造器就有生产级队列。
...
| Feature | 默认 | 启用什么 |
|---------|------|---------|
| `pg`    | on   | `PgQueueStore`, `KevyNotifier`, 自带的 `DeliveryWorker`。引入 `sqlx` (Postgres) + `redis`。 |
