...
- **Postgres リファレンス** —— [`PgQueueStore`] + [`KevyNotifier`] (デフォルト `pg` feature) が mailrs と同じスキーマを直接ターゲット。コンストラクタ 1 つで本番品質のキューが手に入る。
...
|---------|------------|---------------|
| `pg`    | on         | `PgQueueStore`, `KevyNotifier`, 同梱の `DeliveryWorker`。`sqlx` (Postgres) と `redis` を引き入れる。 |
