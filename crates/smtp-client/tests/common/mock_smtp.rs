...
/// serves a single connection with the configured [`Behavior`] and then
/// stops accepting. Returns the bound address + task handle.
pub async fn spawn_mock_smtp(behavior: Behavior) -> MockHandle {
    ...
    let task = tokio::spawn(async move {
        if let Ok((stream, _peer)) = listener.accept().await {
            handle_connection(stream, behavior).await;
        }
    });
