...
use common::{
    InMemoryAddressBookStore, TEST_USER, body_as_str, header_value, make_book, make_contact,
};
...
async fn addressbook_home_propfind_depth_zero_returns_only_home_collection() {
    let store = InMemoryAddressBookStore::new()
        .with_book(TEST_USER, make_book(1, "Friends"));

    let resp = addressbook_home_propfind(&store, TEST_USER, 0)
        .await
        .unwrap();
    let body = body_as_str(resp.body);

    assert_eq!(resp.status, 207);
    assert!(body.contains(&format!("<D:href>/dav/contacts/{TEST_USER}/</D:href>")));
...
async fn addressbook_home_propfind_depth_one_lists_child_books() {
    ...
    let store = InMemoryAddressBookStore::new().with_book(TEST_USER, book);

    let resp = addressbook_home_propfind(&store, TEST_USER, 1)
        .await
        .unwrap();
    let body = body_as_str(resp.body);

    assert!(body.contains(&format!("/dav/contacts/{TEST_USER}/Friends/")));
...
async fn addressbook_home_propfind_only_returns_books_for_requested_user() {
    let store = InMemoryAddressBookStore::new()
        .with_book(TEST_USER, make_book(1, "Mine"))
    ...
    assert!(body.contains(&format!("/dav/contacts/{TEST_USER}/Mine/")));
...
async fn addressbook_propfind_depth_zero_returns_collection_only() {
    let store = InMemoryAddressBookStore::new()
        .with_book(TEST_USER, make_book(20, "Friends"))
        .with_contact(20, make_contact("ct-1", "BEGIN:VCARD\nUID:ct-1\nEND:VCARD"));

    let resp = addressbook_propfind(&store, TEST_USER, "Friends", 20, 0)
        .await
        .unwrap();
    let body = body_as_str(resp.body);
...
async fn addressbook_propfind_depth_one_lists_contact_etags_without_vcard() {
    ...
    let resp = addressbook_propfind(&store, TEST_USER, "Friends", 20, 1)
        .await
        .unwrap();
    let body = body_as_str(resp.body);

    assert!(body.contains(&format!("/dav/contacts/{TEST_USER}/Friends/ct-1.vcf")));
...
async fn addressbook_report_multiget_filters_by_uid() {
    ...
    let body = format!(
        "<CR:addressbook-multiget xmlns:CR=\"urn:ietf:params:xml:ns:carddav\">\
         <D:href>/dav/contacts/{TEST_USER}/Friends/a.vcf</D:href>\
         </CR:addressbook-multiget>"
    );
    let resp = addressbook_report(&store, TEST_USER, "Friends", 20, &body)
        .await
        .unwrap();
