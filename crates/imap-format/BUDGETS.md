# mailrs-imap-format performance budgets

Regression budgets enforced by `tests/perf_gate.rs`.
Run `cargo test -p mailrs-imap-format --test perf_gate` to check.
Run `cargo bench -p mailrs-imap-format` for criterion baselines.
