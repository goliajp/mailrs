...
fn parse_command_complex_uid_search_under_budget() {
    let cmd = "a004 UID SEARCH SINCE 1-Jan-2026 NOT DELETED OR FROM alice@example.com SUBJECT urgent\r\n";
    let median = time_median(|| {
        let _ = parse_command(cmd);
    });
    // Budget: 200 µs. Observed P95: ~5 µs.
    let budget = Duration::from_micros(200);
    assert!(median < budget, "parse_command(complex UID SEARCH) median {median:?} exceeded {budget:?}");
}
...
fn sequence_set_expand_to_uids_under_budget() {
    ...
    assert!(median < budget, "sequence_set_to_uids median {median:?} exceeded {budget:?}");
}
...
fn format_fetch_4_items_under_budget() {
    ...
    assert!(median < budget, "format_fetch median {median:?} exceeded {budget:?}");
}
