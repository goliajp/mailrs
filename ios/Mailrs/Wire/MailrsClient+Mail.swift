...
/// answers how many went through **and which ones did not**. This
/// client used to send one request per row: archiving fifty rows was
/// fifty round trips, and on a phone that is the difference between an
/// action and a wait.
///
/// The per-row loop existed for a reason — it was the only way to know
/// *which* rows failed, and putting the right ones back is the whole
/// job of a batch that half-worked. The route reports that now.
extension MailrsClient {
